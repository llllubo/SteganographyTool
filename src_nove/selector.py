...
class Selector:
    ...
    @staticmethod
    def __compute_avg_cap(eq_class: EqClassesProcessor) -> float:
    ...
    def __set_eq_class(instr: MyInstruction, eq_class_name: str) -> None:
        ...
        print(f"..{eq_class_name} => {instr.set_eq_class}")
    ...
    def select(cls,
        ...
        for my_instr in all_my_instrs:
                ...
                if instr.mnemonic == Mnemonic.MOV:
                    if mov_indicator:
                        ...
                        if cls.__check_independency(prev_mov.instruction,
                            ...
                            prev_mov.set_eq_class = \
                                EqClassesProcessor("MOV Scheduling", "", "",[])
                ...
                if op_code.is_nop and instr.len == 1:
                    ...
                    if nop90_indicator > 0:
                        ...
                        if nop90_indicator == 1:
                            ...
                            # Compute capacities.
                            cap = cls.__compute_avg_cap(my_instr.eq_class)
                            avg_cap += cap
                            ...
                            cap = cls.__compute_avg_cap(
                                selected_my_instrs[-1].eq_class)
                            avg_cap -= cap
                            min_cap -= math.floor(cap)
                            max_cap -= math.ceil(cap)
            ...
            if method == "sub" or \
                ...
                if re.match(
                        r'(?:TEST|AND|OR) r/m[0-9]{1,2}, r[0-9]{1,2}',
                        op_code.instruction_string
